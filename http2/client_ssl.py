...
async def download_file(client, url, timeout=30, max_retries=3):
...
async def main_async(server, file):
    ...
    # Using HTTPS port
    port = 8443
    server_url = f"https://{server_ip}:{port}"
    
    # Client configuration for HTTP/2 over SSL
    client_kwargs = {
        "http2": True,
        "verify": False,
        "limits": httpx.Limits(
            max_connections=10,  # Allow more simultaneous connections
    ...
    # Create a new client for each file size to prevent connection issues
    for exp in experiments:
        click.echo(click.style(f"\n🔄 Creating new connection for {file}_{exp['size']}", fg='cyan'))
        
        # Create fresh client for each file size
